...
st.info(f"📅 발주 대상일: **{weekday_kr[current_weekday]}요일** | 30일치 데이터가 한눈에 들어오도록 표 스타일을 콤팩트하게 조정했습니다.")
...
styled_df = display_df.style.set_properties(**{'font-size': '12px'}).hide(subset=["excel_key"], axis=1)
...
if os.path.exists(HISTORY_FILE):
    ...
    if not raw_history.empty:
        transposed_df = raw_history.set_index("날짜").T.reset_index().rename(columns={"index": "excel_key"})
        meta_df = res[["excel_key", "구분2"]].copy()
        pivot_view = pd.merge(meta_df, transposed_df, on="excel_key", how="left").fillna(0)
        
        d_cols = [c for c in pivot_view.columns if c not in ["excel_key", "구분2"]]
        pivot_view["합계"] = pivot_view[d_cols].sum(axis=1).astype(int)
        pivot_view["평균"] = pivot_view[d_cols].mean(axis=1).round().astype(int)
        
        st.dataframe(pivot_view.style.set_properties(**{'font-size': '11px'}), use_container_width=True, hide_index=True)
        
        with st.expander("🗑️ 데이터 삭제"):
            target = st.selectbox("삭제할 날짜", raw_history["날짜"].tolist())
            if st.button("❌ 삭제 실행"):
                raw_history[raw_history["날짜"] != target].to_csv(HISTORY_FILE, index=False)
                st.rerun()
