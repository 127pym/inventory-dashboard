...
with col3: avg_days = st.number_input("평균 산출 기간(일)", min_value=1, max_value=30, value=10)
with col4:
    file_date = st.date_input("업로드 기준 날짜", datetime.date(2026, 8, 18))
    uploaded_file = st.file_uploader("출고일마감 파일 업로드", type=["xlsx", "xls"])

lead_time_days = max(0, (delivery_date - order_date).days)

# 3. 파일 업로드 및 피벗 집계 (운송장 번호 중복 제거)
...
def calculate_recent_average(n_days):
    ...
    for key in st.session_state.stock_data["excel_key"]:
        vals = [st.session_state.usage_history[d].get(key, 0) for d in target_dates]
        averages.append(round(sum(vals)/len(vals), 1))
    return averages

st.session_state.stock_data["평균사용량"] = calculate_recent_average(int(avg_days))
