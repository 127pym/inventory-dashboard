...
# 파일이 존재하면 불러오고, 없으면 기본값으로 생성하여 영구 보존
...
st.title("📦 물류 재고 및 발주 통합 대시보드 (영구 저장 및 자동 불러오기)")
...
st.info(f"📅 발주 대상일: **{weekday_kr[current_weekday]}요일** | 재고 및 누적 데이터가 시스템에 영구 자동 저장됩니다.")
...
if st.button("🚀 계산 실행 및 데이터 저장", type="primary", use_container_width=True):
    with st.spinner("⚙️ 데이터 분석 및 영구 저장 중..."):
        ...
        # 세션 상태 업데이트 및 로컬 파일에 영구 저장 (CSV)
        st.session_state.stock_data = res
        res.to_csv(DATA_FILE, index=False)
        
        st.session_state.calculated_result = res
        st.success("✅ 계산이 완료되었으며, 모든 데이터가 안전하게 영구 저장되었습니다!")

# --- [고정 틀 5: 계산 결과 고정 영역] ---
st.markdown("---")
st.subheader("📊 최종 계산 및 발주 요약 (데이터 영구 보존됨)")
...
if st.session_state.calculated_result is not None:
    ...
    st.info("ℹ️ 데이터를 입력하고 [계산 실행 및 데이터 저장] 버튼을 누르면 결과가 표시되며 파일에 저장됩니다.")
